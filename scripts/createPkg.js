...
const IS_REACT_PKG = argv.r || argv.react
const IS_ESX_PKG = argv.e || argv.esx

if (!IS_ESX_PKG && !IS_REACT_PKG) {
  console.log('You must use one of --react|-r or --esx|-e flags')
  process.exit()
}

const pkgName = argv._[0] || IS_ESX_PKG || IS_REACT_PKG
    ...
    (resolve, reject) => cmd.get(
      ...
      (err, data, stderr) => {
        ...
        if (!err) {
          ...
          pkgJson.set(
            'repository',
            `https://github.com/jaredLunde/essentials/tree/master/packages/${pkgName}`
          )
          pkgJson.set('name', `@essentials/${pkgName}`)
          ...
          cmd.get(
            ...
            async (err, data, stderr) => {
              ...
              if (!err) {
                await fs.copy(
                  path.join(__dirname, `assets/${IS_ESX_PKG ? 'babel.esx.rc' : 'babel.react.rc'}`),
